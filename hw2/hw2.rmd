...
- **Likelihood Function**
  - Input: beta1, beta2, beta3, sigma2, X (predictors), Y (responses)
...
- **Prior for Beta**
  - Input: beta (parameter value), tau2 (variance)
...
- **Prior for Sigma^2**
  - Input: sigma2, a, b (parameters for the gamma distribution)
...
- **Initialize**: Starting values for Beta_1, Beta_2, Beta_3, and sigma2; containers for chains of sampled values and acceptance rates
- **For Each Iteration** (from 1 to nsim + nburn):
  - **Update Beta_1** using Gibbs sampling:
    ...
    - Calculate the posterior distribution of Beta_1 given eta, zeta, and sigma2, then sample from this distribution
  ...
  - **Update Sigma^2** using Metropolis-Hastings:
    - Propose a new value for sigma2 from a gamma distribution
    - Calculate the log-acceptance ratio based on the likelihood and prior probabilities of the proposed and current sigma2
  ...
  - **Store Sampled Values** (after burn-in and according to thinning parameter):
    - Save the current values of Beta_1, Beta_2, Beta_3, and sigma2 to their respective chains
    - Record the acceptance rates for Beta_2, Beta_3, and sigma2 updates
- **Output**: Return the chains of sampled values for Beta_1, Beta_2, Beta_3, and sigma2, along with the acceptance rates

This sampler combines Gibbs sampling for updating Beta_1 and Metropolis-Hastings for updating Beta_2, Beta_3, and sigma2, to perform Bayesian inference on the given model.
...
## Summarize approximations of the posterior expectations
...
- From the correlation matrix, we can see that the correlations is rather high for $\beta_1$, $\beta_2$ 0.93,  $\beta_1$ and $beta_3$ 0.88 and  $\beta_2$ and  $\beta_3$ 0.97 has rather high correlation.
...
list(
  weighted_mean_beta1 = weighted_mean_beta1,
  weighted_mean_beta2 = weighted_mean_beta2,
  weighted_mean_beta3 = weighted_mean_beta3
)
```
...
list(
  weighted_mean_beta1_lv = weighted_mean_beta1_lv,
  weighted_mean_beta2_lv = weighted_mean_beta2_lv,
  weighted_mean_beta3_lv = weighted_mean_beta3_lv
)
```
