...
Solving this integration with default parameters in `si.quad` will results in numerical issues at the tails of distribution. To deal with this, we tuning the `points` parameter to include the high density region. We can roughly observe that this integration has high density within the range of `(-10, 100)`.
...
To sample from this distribution we use an exponential distribution with rate 0.09 as our proposal distribution which has a heavier tail compared to the target $f(x)$. The proposal distribution $q(x)$ is specified as,
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
acf(apply(sample3, 1, D), plot = TRUE)
```
